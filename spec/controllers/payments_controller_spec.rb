...
RSpec.describe PaymentsController do
  ...
  describe 'GET #edit' do
    let(:another_student) { create :student, first_name: 'Bruce', last_name: 'Lee' }
    let(:payment) { create :payment, student_id: another_student.id  }
  ...
  describe 'PUT #update' do
    ...
    let(:payment_date) { '1993-05-14 21:36:00 UTC' }
    let!(:params) do
      { id: payment.id, payment: { payment_date: payment_date, student_id: student.id } }
    end
    ...
    context 'success' do
      ...
      context 'updates payment' do
        subject { -> { put :update, params } }
        it { is_expected.to change{ payment.reload.payment_date.to_s }.to(payment_date) }
        it { is_expected.to change{ payment.reload.student_id }.to(student.id) }
      end
    end
